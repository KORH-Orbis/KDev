...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    await prisma.contactMessage.create({
      data: { name: nombre, email, phone: telefono, service: servicio, message: mensaje },
    });

    if (resend) {
      await resend.emails.send({
        from: `${process.env.CONTACT_FROM_NAME || "KDev"} <${process.env.CONTACT_FROM_EMAIL || "onboarding@resend.dev"}>`,
        to: process.env.CONTACT_TO_EMAIL || "rodr.kevin99@gmail.com",
    ...
    return res.status(200).json({ data: { success: true, message: "Mensaje recibido correctamente" } });
  } catch (error) {
